...
# Set input params
setwd("/home/tml5905/Documents/HunterGatherFarmerInteractions/cluster_runs/FK_20x_HGK_1chr_mvmt5km_assumption_removed")
map_size_km = 3700
...
remaining_ancestry = ggplot(all_sim_data[TotalHGs == 0 & Learning_Prob_n != 0.00005]) + 
  geom_point(aes(as.numeric(Assortative_Mating_n), Farmer_Ancestry_All_Farmers, col = factor(Learning_Prob_n))) + 
  theme_bw() + geom_line(aes(as.numeric(Assortative_Mating_n), Farmer_Ancestry_All_Farmers, col = factor(Learning_Prob_n), 
                             group = factor(Learning_Prob_n))) + ylim(0.0, 1.0) + labs(x = "Percentage of Assortative Mating (0 = None, 1 = Full)") + 
  labs(y = "Remaining Farming Ancestry") + ggtitle("Remaining Farmer Ancestry", 
                                                   subtitle = "(When Zero Hunter Gatherers Remain)")
ggsave("remaining_farming_ancestry.png", plot = remaining_ancestry, units = "in", width = 10, height = 8, device="png", dpi=700)

overall_ancestry_farmers = ggplot(all_sim_data[Learning_Prob_n != 0.00005]) + 
  geom_line(aes(as.numeric(Year), Farmer_Ancestry_All_Farmers, col = factor(Learning_Prob_n))) + 
...
ggsave("ancestry_farmers_by_year.png", plot = overall_ancestry_farmers, units = "in", width = 10, height = 11, device="png", dpi=700)

overall_ancestry_all = ggplot(all_sim_data[Learning_Prob_n != 0.00005]) + 
  geom_line(aes(as.numeric(Year), Farmer_Ancestry_All, col = factor(Learning_Prob_n))) + 
  theme_bw() + facet_grid(as.numeric(Assortative_Mating_n) ~.) + labs(x = "Year") + 
  labs(y = "Farming Ancestry") + ggtitle("Farming Ancestry x Time (Whole Population)", 
                                         subtitle = "Faceted by Assortative Mating Preference")
ggsave("ancestry_all_inds_by_year.png", plot = overall_ancestry_all, units = "in", width = 10, height = 11, device="png", dpi=700)
...
if (length(square_file_names) != 0)
{
  speed = ggplot(all_sim_data[Learning_Prob_n != 0.00005]) + 
    geom_point(aes(as.numeric(Assortative_Mating_n), speedOfWave, col=factor(Learning_Prob_n))) + theme_bw() + 
    geom_line(aes(as.numeric(Assortative_Mating_n), speedOfWave, col=factor(Learning_Prob_n))) + theme_bw() +
    labs(y = "Speed of Wave (km per year)") + labs(x = "Percentage of Assortative Mating (0 = None, 1 = Full)") +
    ggtitle("Speed of Wave (km per year)")
  ggsave("speed_of_wave.png", plot = speed, units = "in", width = 8, height = 6, device="png", dpi=700)
  
  ancestry = 
    ggplot(all_sim_data[Year %% 200 == 0 & Learning_Prob_n != 0.00005]) + 
  ...
  ggsave("ancestry_farmers_by_distance_and_year.png", plot = ancestry, units = "in", width = 14, height = 8, device="png", dpi=700)
  ...
  ancestry_whole_pop = 
    ...
    labs(y = "Percent Farming Ancestry in Population") + ggtitle("Farming Ancestry x Distance from Origin Point (Whole Population)", 
                                                                 subtitle = "Faceted by Learning Probability ~ Assortative Mating Preference")
  ggsave("farming_ancestry_by_distance_and_year.png", plot = ancestry_whole_pop, units = "in", width = 14, height = 8, device="png", dpi=700)
  
  wave = ggplot(all_sim_data[Year %% 200 == 0 & Learning_Prob_n != 0.00005]) + 
    ...
    labs(y = "Percent Farmers") + ggtitle("Percent Farmers (Behaviorally) x Distance from Origin Point", 
                                          subtitle = "Faceted by Learning Probability ~ Assortative Mating Preference")
  ggsave("wave_of_advance_by_distance_and_year.png", plot = wave, units = "in", width = 14, height = 8, device="png", dpi=700)
  
  wave_ancestry_overlay = ggplot(all_sim_data[Year %% 500 == 0 & Learning_Prob_n != 0.00005]) + 
    ...
    labs(y = "Percent Farmers") + ggtitle("Percent Farmers (Behaviorally) andN Farming Ancestry x Distance from Origin Point", 
                                          subtitle = "Faceted by Learning Probability ~ Assortative Mating Preference")
...
if (length(ancestry_sample_names) != 0)
{
  ...
  singlular_ancestry_2d = ggplot(ancestry_sample_data[Learning_Prob_n == 0]) + geom_point(aes(Individual_X, Individual_Y, col = Farming_Ancestry, pch=factor(Individual_Z))) + 
  ...
  singlular_learning_2d = ggplot(ancestry_sample_data[Assortative_Mating_n == 1]) + geom_point(aes(Individual_X, Individual_Y, col = Farming_Ancestry, pch=factor(Individual_Z))) + 
    facet_grid(cut(Year, seq(0,6000,2000)) ~ .) + ggtitle("Ancestry Distribution on Landscape", subtitle = "2,000 Year Time Bins") + theme_bw() 
...
# ----------------------------------------------------------------------------------------------------------------
# Extra Temp Code for Plots Comparing Assumption to Non
# ----------------------------------------------------------------------------------------------------------------

comparison_subset = all_sim_data[(Assortative_Mating_n == 0.95 & Learning_Prob_n == 0.0) | 
  (Assortative_Mating_n == 0.95 & Learning_Prob_n == 0.0001) | 
  (Assortative_Mating_n == 0.95 & Learning_Prob_n == 0.001) | 
  (Assortative_Mating_n == 0.95 & Learning_Prob_n == 0.01) | 
  (Assortative_Mating_n == 0.7 & Learning_Prob_n == 0.0) | 
  (Assortative_Mating_n == 0.7 & Learning_Prob_n == 0.0001) | 
  (Assortative_Mating_n == 0.7 & Learning_Prob_n == 0.001) | 
  (Assortative_Mating_n == 0.7 & Learning_Prob_n == 0.01) | 
  (Assortative_Mating_n == 0.99 & Learning_Prob_n == 0.0) | 
  (Assortative_Mating_n == 0.99 & Learning_Prob_n == 0.0001) | 
  (Assortative_Mating_n == 0.99 & Learning_Prob_n == 0.001) | 
  (Assortative_Mating_n == 0.99 & Learning_Prob_n == 0.01) | 
  (Assortative_Mating_n == 0.999 & Learning_Prob_n == 0.0) | 
  (Assortative_Mating_n == 0.999 & Learning_Prob_n == 0.0001) | 
  (Assortative_Mating_n == 0.999 & Learning_Prob_n == 0.001) | 
  (Assortative_Mating_n == 0.999 & Learning_Prob_n == 0.01) | 
  (Assortative_Mating_n == 1.0 & Learning_Prob_n == 0.0) | 
  (Assortative_Mating_n == 1.0 & Learning_Prob_n == 0.0001) | 
  (Assortative_Mating_n == 1.0 & Learning_Prob_n == 0.001) | 
  (Assortative_Mating_n == 1.0 & Learning_Prob_n == 0.01)]

remaining_ancestry = ggplot(comparison_subset[TotalHGs == 0]) + 
  geom_point(aes(as.numeric(Assortative_Mating_n), Farmer_Ancestry_All_Farmers, col = factor(Learning_Prob_n))) + 
  theme_bw() + geom_line(aes(as.numeric(Assortative_Mating_n), Farmer_Ancestry_All_Farmers, col = factor(Learning_Prob_n), 
                             group = factor(Learning_Prob_n))) + labs(x = "Percentage of Assortative Mating (0 = None, 1 = Full)") + 
  labs(y = "Remaining Farming Ancestry") + ggtitle("Remaining Farmer Ancestry", 
                                                   subtitle = "(When Zero Hunter Gatherers Remain)")
ggsave("remaining_farming_ancestry_subset_for_comparison.png", plot = remaining_ancestry, units = "in", width = 10, height = 8, device="png", dpi=700)

ancestry_whole_pop = ggplot(comparison_subset[Year %% 200 == 0]) + 
  geom_line(aes(Mid_Point_km, Farmer_Ancestry_Partition_All, col = Year, group = factor(Year))) + 
  facet_grid(as.numeric(Assortative_Mating_n) ~ Learning_Prob_n) + theme_bw() + labs(x = "Distance From Origin (km)") +
  labs(y = "Percent Farming Ancestry in Population") + ggtitle("Farming Ancestry x Distance from Origin Point (Whole Population)", 
                                                               subtitle = "Faceted by Learning Probability ~ Assortative Mating Preference")
ggsave("farming_ancestry_by_distance_and_year_subset_for_comparison.png", plot = ancestry_whole_pop, units = "in", width = 14, height = 8, device="png", dpi=700)

speed = ggplot(comparison_subset) + 
  geom_point(aes(as.numeric(Assortative_Mating_n), speedOfWave, col=factor(Learning_Prob_n))) + theme_bw() + 
  geom_line(aes(as.numeric(Assortative_Mating_n), speedOfWave, col=factor(Learning_Prob_n))) + theme_bw() +
  labs(y = "Speed of Wave (km per year)") + labs(x = "Percentage of Assortative Mating (0 = None, 1 = Full)") +
  ggtitle("Speed of Wave (km per year)")
ggsave("speed_of_wave_subset_for_comparison.png", plot = speed, units = "in", width = 8, height = 6, device="png", dpi=700)

ancestry_whole_pop = 
  ggplot(all_sim_data[Year %% 200 == 0]) + 
  geom_line(aes(Mid_Point_km, Farmer_Ancestry_Partition_All, col = Year, group = factor(Year))) +
  scale_color_gradient(low = "#ab520d", high = "#ecd3bb") + 
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank(), axis.line = element_line(colour = "black")) +
  facet_grid(as.numeric(Assortative_Mating_n) ~ Learning_Prob_n)+ labs(x = "Distance From Origin (km)") + 
  labs(y = "Percent Farming Ancestry in Population") + ggtitle("Farming Ancestry x Distance from Origin Point (Whole Population)", 
                                                               subtitle = "Faceted by Learning Probability ~ Assortative Mating Preference")
ggsave("farming_ancestry_by_distance_and_year.png", plot = ancestry_whole_pop, units = "in", width = 14, height = 8, device="png", dpi=700)
