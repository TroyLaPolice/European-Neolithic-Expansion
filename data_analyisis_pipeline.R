...
# Run a linear model to calculate the speed of wave and add as a column
all_sim_data[, speedOfAncestry := lm(Ancestry_Cline ~ Year)$coeff[2], .(Learning_Prob, Assortative_Mating)]
...
ancestry_speed = ggplot(all_sim_data) + 
  geom_point(aes(as.numeric(Assortative_Mating_n), speedOfAncestry, col=factor(Learning_Prob_n))) + theme_bw() + 
...
ggsave("assort_ancestry_by_x.png", plot = assort_ancestry_by_x, units = "in", width = 17, height = 10, device="png", dpi=700)
...
ggsave("learning_ancestry_by_x.png", plot = learning_ancestry_by_x, units = "in", width = 17, height = 10, device="png", dpi=700)
